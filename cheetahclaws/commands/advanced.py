...
from cheetahclaws.tools import _is_in_tg_turn, _is_in_web_turn
...
def cmd_draft(args: str, _state, config) -> bool:
    ...
    _start_tool_spinner()
    try:
        from cheetahclaws.bridges.wechat_smart_reply import generate_candidates
    ...
    try:
        ...
        if wx_uid:
            from cheetahclaws.bridges.wechat import _wx_send
            from cheetahclaws.bridges.draft_cache import put as _draft_put
...
def cmd_summarize(args: str, _state, config) -> bool:
    ...
    _start_tool_spinner()
    from cheetahclaws.tools.files import _summarize_large_file
    try:
...
def cmd_memory(args: str, _state, config) -> bool:
    ...
    if stripped == "consolidate":
        from cheetahclaws.memory import consolidate_session
...
def cmd_agents(_args: str, _state, config) -> bool:
    try:
        from cheetahclaws.multi_agent.tools import get_agent_manager
...
def cmd_skills(_args: str, _state, config) -> bool:
    from cheetahclaws.skill import load_skills
    skills = load_skills()
    if not skills:
...
def cmd_mcp(args: str, _state, config) -> bool:
    ...
    from cheetahclaws.mcp_client.client import get_mcp_manager
    from cheetahclaws.mcp_client.config import (load_mcp_configs, add_server_to_user_config,
                                remove_server_from_user_config, list_config_files)
    from cheetahclaws.mcp_client.tools import initialize_mcp, reload_mcp, refresh_server
...
def cmd_plugin(args: str, _state, config) -> bool:
    ...
    if subcmd == "recommend":
        context = rest
        if not context:
            from cheetahclaws.plugin.recommend import recommend_from_files
...
def cmd_tasks(args: str, _state, config) -> bool:
    ...
    from cheetahclaws.task import list_tasks, get_task, create_task, update_task, delete_task, clear_all_tasks
    from cheetahclaws.task.types import TaskStatus
