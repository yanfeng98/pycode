...
def cmd_help(_args: str, _state, config) -> bool:
    try:
        # The usage help lives on the CLI module's docstring, not the
        # (deliberately light) package __init__.
...
def cmd_context(_args: str, state, config) -> bool:
    ...
    sys_tokens = 0
    try:
        from cheetahclaws import context as _ctx
...
def cmd_doctor(args: str, state, config) -> bool:
    ...
    import subprocess as _sp
    from cheetahclaws.providers import PROVIDERS, detect_provider, get_api_key
...
def _proactive_rpc(method: str, params: dict | None = None) -> dict | None:
    ...
    try:
        ...
        import json
        import os
        from cheetahclaws.daemon import API_VERSION, API_VERSION_HEADER, discovery
...
def cmd_circuit(args: str, state, config) -> bool:
    ...
    """
    from cheetahclaws import circuit_breaker as _cb
