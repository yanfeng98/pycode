...
This page is the single reference for pycode' security model and every
environment variable that gates a sensitive subsystem. Read this before you
expose pycode to a remote bridge, a multi-user web deployment, or an
untrusted plugin.

## Threat model — at a glance

pycode is built around a single trusted operator running on a single
...
| Remote bridge `!cmd` (Telegram / Slack / WeChat) | **Enabled by default**, owner-only (chat_id whitelist) + hard denylist | `PYCODE_BRIDGE_TERMINAL=0` to disable entirely |
| File I/O (`Read` / `Write` / `Edit`) | CWD-relative, credential paths denied | `allowed_root` config / `PYCODE_FS_NO_SANDBOX=1` |
| Plugins (`plugin/loader.py`) | Manifest-driven, runs arbitrary Python | `PYCODE_DISABLE_PLUGINS=1` / `PYCODE_PLUGIN_ALLOWLIST=…` |
| MCP server env | Dangerous keys (`LD_PRELOAD`, `PYTHONPATH`, …) stripped | `PYCODE_MCP_TRUST_ENV=1` to allow |
...
|---|---|---|
| `TELEGRAM_BOT_TOKEN` | (unset) | Recommended way to provide the Telegram bot token. Token from env never lands in `readline` history or `~/.pycode/config.json`. Beats the `/telegram <token> <chat_id>` syntax. |
| `SLACK_BOT_TOKEN` | (unset) | Same idea for Slack (`xoxb-…`). |
| `QQ_SECRET` | (unset) | QQ bot AppSecret (qq-botpy). Env-supplied secret never lands in `readline` history or `~/.pycode/config.json`. Beats the `/qq <appid> <secret>` syntax. |
| `QQ_APPID` | (unset) | QQ bot AppID — a public identifier, not sensitive. Optional convenience so `/qq` works with no args. |
| `PYCODE_BRIDGE_TERMINAL` | `1` | Set to `0` to hard-disable remote `!cmd` shell from any bridge (Telegram / Slack / WeChat / QQ). Useful when a bridge owner is *not* the same person as the host operator. |
| `PYCODE_FS_NO_SANDBOX` | `0` | Bypass the credential-path denylist (SSH private keys, `~/.aws`, `~/.gnupg`, `/etc/shadow`, etc.). Only set if you're deliberately auditing your own secrets. |
| `PYCODE_DISABLE_PLUGINS` | `0` | Hard-disable plugin loading regardless of what's installed in `~/.pycode/plugins/`. |
| `PYCODE_PLUGIN_ALLOWLIST` | (unset) | Comma-separated plugin names. When set, only these plugins are loaded — everything else is silently skipped, even if enabled in the registry. |
| `PYCODE_MCP_TRUST_ENV` | `0` | Set to `1` to allow MCP server configs to inject `LD_PRELOAD`, `LD_LIBRARY_PATH`, `DYLD_*`, `PYTHONPATH`, `PYTHONSTARTUP`, `PYTHONHOME`, `NODE_OPTIONS`, `NODE_PATH`, `BASH_ENV`, or `ENV`. Default is to silently strip these from `mcp.json`-supplied env. |
| `PYCODE_WEB_SECRET` | (unset) | Per-deployment JWT signing secret for the web UI. Strongly recommended for production — overrides the auto-generated file at `~/.pycode/web_secret`. |

## Bot tokens — recommended setup

Tokens passed as `/telegram <TOKEN> <chat_id>` in the REPL land in your
`readline` history file (`~/.pycode/input_history.txt`). Subsequent
...
If you do use the old two-arg form, pycode will:
...
via QR-code scan and stored in `~/.pycode/config.json` (0600).
...
multi-user deployments and is what `pycode daemon` uses.
...
Set `PYCODE_FS_NO_SANDBOX=1` to disable the denylist (e.g. when
...
| JWT signing secret | `secrets.token_urlsafe(32)` saved to `~/.pycode/web_secret` with `O_CREAT \| O_EXCL` + 0o600. If the post-write `stat` shows world-readable mode the file is removed and the secret falls back to in-memory only; override with `PYCODE_WEB_SECRET`. |

## Plugins

Plugins are loaded by `importlib` from `~/.pycode/plugins/<name>/`
or any directory on `$PYCODE_PLUGIN_PATH`. Each module runs with
the same privileges as the pycode process — there is no sandbox.

Mitigations on by default:

- Module paths are confined to the plugin's `install_dir` (no
  `../../etc/passwd` traversal).
- EXTERNAL-scope plugins (loaded via `$PYCODE_PLUGIN_PATH`) print a
  one-time stderr warning on first load so a stolen env var doesn't
  load code silently.

Use `PYCODE_DISABLE_PLUGINS=1` for an emergency kill switch or
`PYCODE_PLUGIN_ALLOWLIST=a,b,c` to whitelist exactly the plugins
you trust.

## MCP

MCP server configs (`.mcp.json`, plugin manifests) supply a `command`,
`args`, and an `env` map. pycode merges `env` over `os.environ`
...
keys …` notice to stderr. Set `PYCODE_MCP_TRUST_ENV=1` if a
...
`~/.pycode/config.json`, so launching pycode again starts
