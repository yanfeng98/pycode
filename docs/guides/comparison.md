# Comparison: PyCode vs Claude Code & OpenClaw
...
**PyCode** reimplements the same core loop in ~90K lines of readable Python (core, excluding tests), keeping everything you need and dropping what you don't.
...
| Build step required | Yes (Bun + esbuild) | No — run directly with `python pycode.py` (or install to use `pycode`) |
...
- **Telegram Bot Bridge** — `/telegram <token> <chat_id>` turns pycode into a Telegram bot: receive user messages, run the model, and send back responses — all from your phone. Slash commands pass through, and a typing indicator keeps the chat feeling live.
...
- **Coding toolset** — Read/Write/Edit/Bash/Glob/Grep/NotebookEdit/GetDiagnostics are purpose-built for software development; PyCode understands diffs, file trees, and code structure.
...
- **Zero setup** — `pip install cheetahclaws` and run `pycode`; no daemon, no pairing, no onboarding wizard.
...
| An AI coding assistant in your terminal | **PyCode** |
| Full offline / local model support | **PyCode** |
| A mobile-friendly always-on experience | **OpenClaw** |
| To read and modify the source in an afternoon | **PyCode** |
| Browser automation and a visual Canvas | **OpenClaw** |
| Multi-provider LLM switching without rebuilding | **PyCode** |

---

## Key design differences

**Agent loop** — PyCode uses a Python generator that `yield`s typed events (`TextChunk`, `ToolStart`, `ToolEnd`, `TurnDone`). The entire loop is visible in one file, making it easy to add hooks, custom renderers, or logging.
